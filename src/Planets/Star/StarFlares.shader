...
uniform float time_speed = 0.05; //hint_range(0.0, 1.0)
uniform float time;
uniform float rotation = 0.0; //hint_range(0.0, 6.28)
uniform bool should_dither = true;

uniform float storm_width = 0.3; //hint_range(0.0, 0.5)
uniform float storm_dither_width = 0.07; //hint_range(0.0, 0.5)

uniform float scale = 1.0;
uniform float seed; //hint_range(1, 10)
uniform float circle_amount = 5.0; //hint_range(2.0, 30.0)
uniform float circle_scale = 1.0; //hint_range(0.0, 1.0)
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords){
	// pixelize uv
	vec2 pixelized = floor(texture_coords*pixels)/pixels;
	
	// use dither val later to interpolate between alpha
	bool dith = dither(texture_coords, pixelized);
	...
	vec2 circletexture_coords = vec2(d, angle);
	
	// two types of noise values
	float n = fbm(circletexture_coords*size -time * time_speed);
	float nc = circle(circletexture_coords*scale -time * time_speed + n);
	
	nc *= 1.5;
	float n2 = fbm(circletexture_coords*size -time + vec2(100, 100));
	nc -= n2 * 0.1;
	...
	vec4 col = Texel(colorramp, vec2(interpolate, 0.0));
